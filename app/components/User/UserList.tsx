...
const UserList: React.FC<UserListProps> = ({ users, onDelete }) => {
  ...
  return (
    <div className="w-full md:w-[70%] mx-auto p-4 mt-4 min-h-screen">
      {/* Navigation Links */}
      <div className="w-full flex flex-col sm:flex-row justify-end items-center space-y-2 sm:space-y-0 sm:space-x-4 mb-4">
        <Link
          href="/admin"
          className="flex items-center justify-center w-full sm:w-28 h-10 bg-blue-500 text-white rounded-md shadow-md hover:bg-blue-600"
        >
          Back
        </Link>
        <Link
          href="/admin/addedUser"
          className="flex items-center justify-center w-full sm:w-40 h-10 bg-blue-500 text-white rounded-md shadow-md hover:bg-blue-600"
        >
          Add New User
        </Link>
      </div>
      ...
      <ul className="space-y-4 w-full">
        {filteredUsers.map(user => {
          ...
          return (
            <li
              key={user.id}
              className="flex flex-col md:flex-row items-center justify-between p-4 border rounded-lg shadow-sm hover:bg-gray-100 w-full"
            >
              {/* User Details */}
              <div className="flex flex-col md:flex-row items-start md:items-center w-full md:w-[60%] mb-2 md:mb-0">
                <h3 className="text-xl font-semibold">{user.user.username}</h3>
                <p className="text-sm text-gray-500 md:ml-4">email: {user.user.email}</p>
              </div>
              {/* Action Links */}
